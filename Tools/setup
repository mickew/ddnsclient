...
####### Variables
BASEDIR=${PWD}
application_name="Dynamic DNS Client"
app_dir="ddnsclient"
install_dir="/usr/local/bin/$app_dir"
app="DdnsClient"

service="ddnsclient.service"
serviceurl="https://raw.githubusercontent.com/mickew/ddnsclient/main/Tools/$service"

latesturl="https://github.com/mickew/ddnsclient/releases/latest"
rooturl="https://github.com/mickew/ddnsclient/releases/download"
...
if [ "$(id -u)" != "0" ]; then
   ...
   lod "This script must be run as root" 1>&2
   exit 1
fi
...
function install {
    ...
    if checkifinstalled; then
        ...
        if [ -n "$APIKEY" ]; then
            ...
            whiptail --title "$application_name" --msgbox "No apikey enterd, cant install." 8 78
        fi
